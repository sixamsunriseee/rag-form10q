...
def generate_chunks(size: int, overlap: int, input_directory: str, input_suffix: str, output_file: str):
    ...
    for filename in os.listdir(input_directory):
        ...
        file_chunks = (
            Chunk(index=i+1, filename=filename, content=chunk, route=Route(company=company, year=int(year), quarter=quarter))
            for i, chunk in enumerate(chunker.split_to_chunks(contents))
        )
...
async def load_chunks_to_database(
    ...
    if batch_size:
        for batch in batched(chunks, batch_size):
            await database.upsert_chunks(collection_name, list(batch))
...
async def answer_questions(
    ...
    for batch in batched(questions, batch_size):
        tasks = [
            run_inference(database, collection_name, retrieve_limit, llm, question)
